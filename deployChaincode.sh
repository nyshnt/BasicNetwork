...
presetup() {
    ...
    pushd ./artifacts/src/github.com/fabuser/go
...
VERSION="1"
CC_SRC_PATH="./artifacts/src/github.com/fabuser/go"
CC_NAME="fabuser"
...
chaincodeInvoke() {
    ...
    ## Add private data
    export CAR=$(echo -n "{\"key\":\"1111\", \"name\":\"Test\",\"email\":\"test@gmail.com\",\"country\":\"India\"}" | base64 | tr -d \\n)
    peer chaincode invoke -o localhost:7050 \
        --ordererTLSHostnameOverride orderer.example.com \
        --tls $CORE_PEER_TLS_ENABLED \
        --cafile $ORDERER_CA \
...
chaincodeQuery() {
    ...
    # Query Car by Id
    peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"function": "queryUser","Args":["CAR0"]}'
